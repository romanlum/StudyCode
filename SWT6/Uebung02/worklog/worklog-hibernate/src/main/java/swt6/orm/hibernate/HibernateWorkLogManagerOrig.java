...
public class HibernateWorkLogManagerOrig {
    ...
    for (Employee e : emplList) {
      ...
      if (e.getLogbookEntries().size() > 0) {
        ...
        for (LogbookEntry lbe : e.getLogbookEntries()) {
          System.out.println("    " + lbe.getActivity() + ": " + lbe.getStartTime() + " - " + lbe.getEndTime() + " - "+lbe.getPhase().getName());
        }
      }
  ...
  private static void getEmployee(long emplId) {
    ...
    if (empl != null) {
      ...
      for (LogbookEntry lbe : empl.getLogbookEntries()) {
        System.out.println("    " + lbe.getActivity() + ": " + fmt.format(lbe.getStartTime()) + " - " + fmt.format(lbe.getEndTime()));
      }
    }
  ...
  private static void testFetchingStrategies() {
    ...
    Long emplId = null;
    try {
      Iterator<LogbookEntry> entryIt = session.createQuery("select le from LogbookEntry le").setMaxResults(1).iterate();
      if (!entryIt.hasNext()) {
        return;
      }
  ...
  public static void testCascade() {
    Employee empl1 = new Employee("Franz", "Mayr", DateUtil.getDate(1980, 12, 24), new Address("4232", "Hagenberg", "Hauptstraße 1"));
    Employee empl2 = new Employee("Fritzi", "Huber", DateUtil.getDate(1980, 12, 24), new Address("4232", "Hagenberg", "Hauptstraße 1"));
    saveEntity(empl1);
    saveEntity(empl2);
    ...
    e1.setModule(new Module("Module1"));
    e2.setModule(new Module("Module1"));
