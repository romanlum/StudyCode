...
public class GameController {
    ...
    public String processUpdate(@PathVariable("gameId") Long gameId,
                                @ModelAttribute("entry") Game game,
                                BindingResult result,
                                Model model){

        if (gameId == null || result.hasErrors()){
            model.addAttribute("teams",soccerService.findAllTeams());
            return "games/manage";
        }

        try {
            //only goals are available in the entry because disabled fields are not submitted
            soccerService.addGameResults(gameId,game.getGoalsA(),game.getGoalsB());
