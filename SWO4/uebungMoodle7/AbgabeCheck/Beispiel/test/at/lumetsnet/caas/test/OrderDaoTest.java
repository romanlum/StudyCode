...
public class OrderDaoTest extends GenericDaoTest {
	...
	public void setUp() {
		...
		Menu menu = new Menu(-1, "desc", 1000, LocalDate.now(), LocalDate.now()
				.plusDays(100), null);
		menu.setCategoryId(cat.getId());
		menuDao.saveOrUpdate(menu);
		menuId = menu.getId();

		User user = new User(-1, "username", "pass", "firstname", "lastname",
				false);
	...
	public void getOrdersByDateTest() {
		...
		entity.setUserId(userId);
		dao.saveOrUpdate(entity);
		entity = new Order(-1, null, null, LocalDateTime.now().plusDays(1),
				"other");
		entity.setMenuId(menuId);
		entity.setUserId(userId);
		dao.saveOrUpdate(entity);
		...
		assertEquals("today", result.stream().findFirst().orElse(null)
				.getComment());
	}
